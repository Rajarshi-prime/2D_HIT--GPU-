...
sig_s_ss = np.zeros((Ntimes, Nprtcl))
sig_n_sn = np.zeros((Ntimes, Nprtcl))
omg_zeta = np.zeros((Ntimes,Nprtcl))
...
for i,t in tqdm(enumerate(times)):
    ...
    uinterp[i,:],Ainterp[:] = interp_spline(pos[i],[u_field,Ar],shp = [(Nprtcl,1),(Nprtcl,1,1)])
    Ainterp *= tp
    ...
    sig_n_sn[i,:] = (Ainterp[:,0,0] - Ainterp[:,1,1])*(Z[:,0,0] - Z[:,1,1])
    sig_s_ss[i,:] = (Ainterp[:,0,1] + Ainterp[:,1,0])*(Z[:,0,1] + Z[:,1,0])
    omg_zeta[i,:] = (Ainterp[:,1,0] - Ainterp[:,0,1])*(Z[:,1,0] - Z[:,0,1])
...
sig_s_ss_c = sig_s_ss[:,causidx]
sig_s_ss_nc = sig_s_ss[:,noncausidx]
del sig_s_ss

sig_n_sn_c = sig_n_sn[:,causidx]
sig_n_sn_nc = sig_n_sn[:,noncausidx]
del sig_n_sn

omg_zeta_c = omg_zeta[:,causidx]
omg_zeta_nc = omg_zeta[:,noncausidx]
del omg_zeta
...
uinterp_caus_shifted = np.ones((maxsize,int(tot_caus),d))*np.nan
...
sig_s_ss_caus_shifted = np.ones((maxsize,int(tot_caus)))*np.nan
sig_n_sn_caus_shifted = np.ones((maxsize,int(tot_caus)))*np.nan
omg_zeta_caus_shifted = np.ones((maxsize,int(tot_caus)))*np.nan
...
for i in range(len(caus_nums)):
    ...
    for j in range(int(caus_count_c[-1,i])):
        ...
        if regionlen > maxsize:
            ...
            uinterp_caus_shifted[:,ins] = (uinterp_c[region,i])[-maxsize:]
            ...
            sig_s_ss_caus_shifted[:,ins] = (sig_s_ss_c[region,i])[-maxsize:]
            sig_n_sn_caus_shifted[:,ins] = (sig_n_sn_c[region,i])[-maxsize:]
            omg_zeta_caus_shifted[:,ins] = (omg_zeta_c[region,i])[-maxsize:]
            ...
            uinterp_caus_shifted[(maxsize-regionlen):,ins] = uinterp_c[region,i]
            ...
            sig_s_ss_caus_shifted[(maxsize-regionlen):,ins] = sig_s_ss_c[region,i]
            sig_n_sn_caus_shifted[(maxsize-regionlen):,ins] = sig_n_sn_c[region,i]
            omg_zeta_caus_shifted[(maxsize-regionlen):,ins] = omg_zeta_c[region,i]
...
del Qinterp_c,caus_count_c,TrZ_c,TrZ2_c,xiinterp_c,ainterp_c,binterp_c,cinterp_c,sig_s_ss_c,sig_n_sn_c,omg_zeta_c,pos_c,vel_c,uinterp_c
#%%
...
sig_s_ss_nc = sig_s_ss_nc[0:,satisfying_conditionidx][:,random_indices]
sig_n_sn_nc = sig_n_sn_nc[0:,satisfying_conditionidx][:,random_indices]
omg_zeta_nc = omg_zeta_nc[0:,satisfying_conditionidx][:,random_indices]
...
with h5py.File(prtcl_loadPath/f"caus-details.hdf5",'a') as f:
    ...
    save_dset(f,"u_caus_shifted",data = uinterp_caus_shifted)
    ...
    save_dset(f,"sig_s_ss_caus_shifted",data = sig_s_ss_caus_shifted)
    save_dset(f,"sig_n_sn_caus_shifted",data = sig_n_sn_caus_shifted)
    save_dset(f,"omg_zeta_caus_shifted",data = omg_zeta_caus_shifted)
...
del Q_caus_shifted,omg_caus_shifted,TrZ_shifted,TrZ2_shifted,Q_caus_shifted_vort,Q_caus_shifted_strain,Q_caus_shifted_extreme_vort,Q_caus_shifted_extreme_strain,Q_caus_shifted_extreme_pos_Q,Q_caus_shifted_extreme_neg_Q,t_caus_len,sig_caus_shifted,a_caus_shifted,b_caus_shifted,c_caus_shifted,sig_s_ss_caus_shifted,sig_n_sn_caus_shifted,omg_zeta_caus_shifted,pos_caus_shifted,vel_caus_shifted,uinterp_caus_shifted
#%%
...
uinterp_nc_shifted = np.zeros((2*maxsize,len(random_indices),d))*np.nan
...
sig_s_ss_nc_shifted = np.zeros((2*maxsize,len(random_indices)))*np.nan
sig_n_sn_nc_shifted = np.zeros((2*maxsize,len(random_indices)))*np.nan
omg_zeta_nc_shifted = np.zeros((2*maxsize,len(random_indices)))*np.nan
...
for i in range(Qinterp_nc.shape[1]):
    ...
    if len(times) - tshiftidx < maxsize: 
        ...
        uinterp_nc_shifted[maxsize:maxsize + len(times) -tshiftidx ,i] = uinterp_nc[tshiftidx:,i]
        uinterp_nc_shifted[idxstart:maxsize,i] = uinterp_nc[idxinit:tshiftidx,i]
        ...
        sig_s_ss_nc_shifted[maxsize:maxsize + len(times) -tshiftidx ,i] = sig_s_ss_nc[tshiftidx:,i]
        sig_s_ss_nc_shifted[idxstart:maxsize,i] = sig_s_ss_nc[idxinit:tshiftidx,i]
        
        sig_n_sn_nc_shifted[maxsize:maxsize + len(times) -tshiftidx ,i] = sig_n_sn_nc[tshiftidx:,i]
        sig_n_sn_nc_shifted[idxstart:maxsize,i] = sig_n_sn_nc[idxinit:tshiftidx,i]
        
        omg_zeta_nc_shifted[maxsize:maxsize + len(times) -tshiftidx ,i] = omg_zeta_nc[tshiftidx:,i]
        omg_zeta_nc_shifted[idxstart:maxsize,i] = omg_zeta_nc[idxinit:tshiftidx,i]
        ...
        uinterp_nc_shifted[maxsize:idxend,i] = uinterp_nc[tshiftidx:idxfin,i]
        uinterp_nc_shifted[maxsize - tshiftidx:maxsize,i] = uinterp_nc[:tshiftidx,i]
        ...
        sig_s_ss_nc_shifted[maxsize:idxend,i] = sig_s_ss_nc[tshiftidx:idxfin,i]
        sig_s_ss_nc_shifted[maxsize - tshiftidx:maxsize,i] = sig_s_ss_nc[:tshiftidx,i]
        
        sig_n_sn_nc_shifted[maxsize:idxend,i] = sig_n_sn_nc[tshiftidx:idxfin,i]
        sig_n_sn_nc_shifted[maxsize - tshiftidx:maxsize,i] = sig_n_sn_nc[:tshiftidx,i]
        
        omg_zeta_nc_shifted[maxsize:idxend,i] = omg_zeta_nc[tshiftidx:idxfin,i]
        omg_zeta_nc_shifted[maxsize - tshiftidx:maxsize,i] = omg_zeta_nc[:tshiftidx,i]
        ...
        uinterp_nc_shifted[maxsize:,i] = uinterp_nc[tshiftidx:tshiftidx+maxsize,i]
        uinterp_nc_shifted[:maxsize,i] = uinterp_nc[tshiftidx-maxsize:tshiftidx,i]
        ...
        sig_s_ss_nc_shifted[maxsize:,i] = sig_s_ss_nc[tshiftidx:tshiftidx+maxsize,i]
        sig_s_ss_nc_shifted[:maxsize,i] = sig_s_ss_nc[tshiftidx-maxsize:tshiftidx,i]
        
        sig_n_sn_nc_shifted[maxsize:,i] = sig_n_sn_nc[tshiftidx:tshiftidx+maxsize,i]
        sig_n_sn_nc_shifted[:maxsize,i] = sig_n_sn_nc[tshiftidx-maxsize:tshiftidx,i]
        
        omg_zeta_nc_shifted[maxsize:,i] = omg_zeta_nc[tshiftidx:tshiftidx+maxsize,i]
        omg_zeta_nc_shifted[:maxsize,i] = omg_zeta_nc[tshiftidx-maxsize:tshiftidx,i]
...
del Qinterp_nc,xiinterp_nc,TrZ_nc,TrZ2_nc,ainterp_nc,binterp_nc,cinterp_nc,sig_s_ss_nc,sig_n_sn_nc,omg_zeta_nc,pos_nc,vel_nc
...
with h5py.File(prtcl_loadPath/f"caus-details.hdf5",'a') as f:
    ...
    save_dset(f,"u_nc_shifted",data = uinterp_nc_shifted)
    ...
    save_dset(f,"sig_s_ss_nc_shifted",data = sig_s_ss_nc_shifted)
    save_dset(f,"sig_n_sn_nc_shifted",data = sig_n_sn_nc_shifted)
    save_dset(f,"omg_zeta_nc_shifted",data = omg_zeta_nc_shifted)
