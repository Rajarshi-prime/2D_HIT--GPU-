...
def plot_noncaus_prtcl(axs,nnoncausprtcl,pos,tidx):
    axs.scatter(pos[tidx,:,0],pos[tidx,:,1],s =20.0,marker = '.',c = "green")
    for prtcl in range(nnoncausprtcl):
        axs.plot(pos[:tidx,prtcl,0],pos[:tidx,prtcl,1],'.',color = "green",markersize = 0.1)
    return None
#%%
...
pos = np.zeros((ntimes,nnoncausprtcl,2))
#%%
prtcl_idx = np.random.choice(prtcl_idx, nnoncausprtcl)
print(ntimes,prtcl_idx)
caus_countf = caus_countf[prtcl_idx]


#%%
tcaus = np.ones_like(caus_countf)*65536
...
for tidx, t in enumerate(np.arange(ti,tf + 0.5*dtload,dtload)):
    ...
    pos[tidx] = np.load(prtcl_loadPath/f"time_{t:.2f}/pos.npz")["pos"][prtcl_idx,:]
    caus_count =  np.round(np.load(prtcl_loadPath/f"time_{t:.2f}/caus_count.npz")["caustics_count"]).astype(np.int32)[prtcl_idx]
    
    caus = caus_count - caus_countf
    if caus.sum() > 0: 
        print("Caustics happened at time", t)
        raise SystemExit 
    ...
    # if True:    
        ...
        for ax in axs.flatten():
            ...
            plot_noncaus_prtcl(ax, nnoncausprtcl, pos, tidx)
        ...
        fig.savefig(savedir/f"noncaustics_time_{t:.0f}_alph_{alph:.2f}.png",bbox_inches='tight')
