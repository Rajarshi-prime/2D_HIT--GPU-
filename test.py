...
st = 0.225
Nprtcl = 314572
alphs = np.array([0.70,0.72,0.75,0.77,0.80,0.85,0.90,1.00])
caus_count = np.array([4189,9623,21033,28505,41678,61961,79789,97474]) #! Particles that went through one or more caustics. 
spc_count = np.array([20830,31949,38787,44361,45008,42222,37516,32955]) #! Particles that went through large Q regions and survived.
# with h5py.File("/home/rajarshi.chattopadhyay/pokemon_data/plot_data.hdf5","r+") as file:
#     file.create_dataset("alphs",data = alphs, dtype = np.float64)
#     f = file[f"st_{round(st,3):.2f}"]
#     f.create_dataset("ncaus",data = caus_count, dtype = np.float64, compression = 'gzip', compression_opts = 9)
#     f.create_dataset("nspl",data = spc_count, dtype = np.float64, compression = 'gzip', compression_opts = 9)
#     file.attrs["Nprtcl"] = Nprtcl
    
# %%
st = 0.3
Nprtcl = 314572
alphs = np.array([0.70,0.72,0.75,0.77,0.80,0.85,0.90,1.00])
caus_count = np.array([41710,75219,120978,147730,179722,217894,243118,270736])
spc_count = np.array([95285,91107,79090,67964,55639,39849,29080,16896])
# with h5py.File("/home/rajarshi.chattopadhyay/pokemon_data/plot_data.hdf5","r+") as file:
#     # file.create_dataset("alphs",data = alphs, dtype = np.float64)
#     try: f = file[f"st_{round(st,3):.2f}"]
#     except: f = file.create_group(f"st_{round(st,3):.2f}")
#     f.create_dataset("ncaus",data = caus_count, dtype = np.float64, compression = 'gzip', compression_opts = 9)
#     f.create_dataset("nspl",data = spc_count, dtype = np.float64, compression = 'gzip', compression_opts = 9)
#%%    
with h5py.File("/home/rajarshi.chattopadhyay/pokemon_data/plot_data.hdf5","r+") as file:
...
# %%
plt.plot(alphs, caus_count/Nprtcl,'o-',color = "blue")
plt.plot(alphs, spc_count/Nprtcl, 'o-', color = "red")
plt.xlabel(r"$\alpha$")
plt.ylabel(r"Fractions of particles")
plt.legend(["Caustics","Survived"])
plt.savefig(f"/home/rajarshi.chattopadhyay/pokemon_data/Plots/Re_125000.0,dt_0.005,N_1024/propensity_st_{st:.2f}.pgf",bbox_inches = "tight",format = "pgf")
# %%
