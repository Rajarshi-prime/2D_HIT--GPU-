...
switch = np.zeros_like(caus_count)
switch[:-1] = caus_count[1:] - caus_count[:-1] 
switch[0] = 1
Q_trajs = np.split((Qinterp.T).ravel(),np.where((switch.T).ravel()>0)[0])
omg_trajs = np.split((xiinterp.T).ravel(),np.where((switch.T).ravel()>0)[0])
TrZ_trajs = np.split((TrZ.T).ravel(),np.where((switch.T).ravel()>0)[0])
TrZ2_trajs = np.split((TrZ2.T).ravel(),np.where((switch.T).ravel()>0)[0])
