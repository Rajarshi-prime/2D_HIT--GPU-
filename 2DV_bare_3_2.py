...
arr_k_padded = np.zeros((int(Nx*3/2),int(Ny*3/4 )+ 1),dtype = np.complex128)
ur = np.empty(((Nx*3)//2,(3*Ny)//2),dtype = np.float64)
vr = np.empty(((Nx*3)//2,(3*Ny)//2),dtype = np.float64)
xi_xr = np.empty(((Nx*3)//2,(3*Ny)//2),dtype = np.float64)
xi_yr = np.empty(((Nx*3)//2,(3*Ny)//2),dtype = np.float64)
advterm = np.empty(((Nx*3)//2,(3*Ny)//2),dtype = np.float64)
factor = np.empty(Nf)
...
## ----------------- setting fft plans --------------------------
plan_f = fft.get_fft_plan(f_r,axes = (0,1),value_type = "R2C")
plan_if = fft.get_fft_plan(xi,axes = (0,1),value_type = "C2R")

plan_padded_f = fft.get_fft_plan(ur,axes = (0,1),value_type = "R2C")
plan_padded_if = fft.get_fft_plan(arr_k_padded,axes = (0,1),value_type = "C2R")



f = fft.rfft2(f_r,plan = plan_f)
def forcing(xi,psi):
    # """
...
## -------- padded ffts and iffts ----------------
kyint = np.round(ky).astype(int)
kxint = np.round(kx).astype(int)
pad_normalize = np.where((ky == N//2),0.5,1)*((3/2)**2)
pad_inormalize = np.where((ky == N//2),2,1)*np.where((kx == -N//2)*(ky ==0),2,1) 
def pad_ifft2(x):
    """Pads the array to 3/2 of the intial size and returns the fourier transform"""
    global arr_k_padded
    arr_k_padded[:] = 0.0
    arr_k_padded[:Nx//2,:Ny//2 + 1] = (x*pad_normalize)[:Nx//2,:Ny//2 + 1]
    arr_k_padded[-Nx//2:,:Ny//2 + 1] = (x*pad_normalize)[-Nx//2:,:Ny//2 + 1]
    return fft.irfft2(arr_k_padded,(3*Nx//2, 3*Ny//2),plan = plan_padded_if)

def pad_fft2(x):
    """Does a fourier transform and cuts of the padded region"""
    return fft.ifftshift(fft.fftshift(fft.rfft2(x,plan = plan_padded_f),axes = 0)[N//4:-N//4,:N//2 + 1],axes  = 0)*((2/3)**2)*pad_inormalize
...
def adv(t,xi,i):
    ...
    # e_arr[:] = e2d_to_1d(0.5*(xi*np.conjugate(psi))*normalize) #!
    # if np.sum(e_arr)>100: raise ValueError("Blowup")
    ur[:] = pad_ifft2(1j * ky*psi)
    vr[:] = pad_ifft2(-1j * kx*psi) 
    xi_xr[:] = pad_ifft2(1j * kx*xi)
    xi_yr[:] = pad_ifft2(1j * ky*xi)
    advterm[:] = ur*xi_xr + vr*xi_yr
    return -1.0*dealias * (pad_fft2(advterm)) + forcing(xi,psi) - xivis*xi*dealias
...
# eprofile[:] = eprofile/np.histogram(k.ravel(),bins = shells)[0][:Nf]
...
xi0_r = fft.irfft2(xi0,(Nx,Ny),plan = plan_if)
...
ur[:] = pad_ifft2(1j * ky*psi0)
vr[:] = pad_ifft2(-1j * kx*psi0) 
e_arr = e2d_to_1d(0.5*(xi0*np.conjugate(psi0))*normalize)
