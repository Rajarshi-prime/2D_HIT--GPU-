...
#! Checking with Taylor Green Vortex
psi_a = fft.rfft2(np.sin(x)*np.sin(y))
u_field_a = u_field*0.0
u_field_a[...,0] = np.sin(x)*np.cos(y)
u_field_a[...,1] = -np.cos(x)*np.sin(y)
k1_a = 0.0*psi

umat_a = np.zeros((Nprtcl,d))
Amat_a = np.zeros((Nprtcl,d,d))
DuDt_mat_a = np.zeros((Nprtcl,d))
grdDuDt_mat_a = np.zeros((Nprtcl,d,d))
## --------------------------------------------------------------
def e2d_to_1d(x):
...
def calc_analytical_fieldx(x):
    global umat_a,Amat_a,DuDt_mat_a,grdDuDt_mat_a
    umat_a[...,0] = np.sin(x[...,0])*np.cos(x[...,1])
    umat_a[...,1] = -np.cos(x[...,0])*np.sin(x[...,1])
    
    Amat_a[...,0,0] = np.cos(x[...,0])*np.cos(x[...,1])
    Amat_a[...,0,1] = -np.sin(x[...,0])*np.sin(x[...,1])
    Amat_a[...,1,0] = np.sin(x[...,0])*np.sin(x[...,1])
    Amat_a[...,1,1] = -np.cos(x[...,0])*np.cos(x[...,1])
    
    DuDt_mat_a[...,0] = np.cos(x[...,0])*np.sin(x[...,0])
    DuDt_mat_a[...,1] = np.sin(x[...,1])*np.cos(x[...,1])
    
    grdDuDt_mat_a[:] = 0.0
    grdDuDt_mat_a[...,0,0] = np.cos(2*x[...,0])
    grdDuDt_mat_a[...,1,1] = np.cos(2*x[...,1])
     
    return umat_a, Amat_a, DuDt_mat_a, grdDuDt_mat_a
...
def initialize_b_spline(order,X,dx,Nx):
    ...
    ck2d = fft.rfft2(cxy)#*dealias #! This will be multiplied with all the fields to be interpolated. 
...
def evolve_and_save(f,t,x0):
    ...
    for i,ti in enumerate(t[:-1]):
        ...
        u_field[:],A_field[:],DuDt_field[:],grdDuDt_field[:] = calc_phys_fields(psi_a,k1_a)
                
        umat[:],Amat[:],DuDt_mat[:],grdDuDt_mat[:] = interp_spline(xprtcl[:,:d],u_field,A_field,DuDt_field,grdDuDt_field)    
        # DuDt_mat[:] = umat[...,None,0]*Amat[...,0] + umat[...,None,1]*Amat[...,1]
        
        umat_a[:],Amat_a[:],DuDt_mat_a[:],grdDuDt_mat_a[:] = calc_analytical_fieldx(xprtcl[:,:d])
        
        count += 1
        uerr += rel_err(umat,umat_a)
        Aerr += rel_err(Amat,Amat_a)
        DuDterr += rel_err(DuDt_mat,DuDt_mat_a)
        grdDuDterr += rel_err(grdDuDt_mat,grdDuDt_mat_a)
        ...
        umat_a[:],Amat_a[:],DuDt_mat_a[:],grdDuDt_mat_a[:] = calc_analytical_fieldx(xtemp[:,:d])
        ...
        caus_count[cond] += 1
        Z_new[cond] *= 0.0
        if np.isnan(Z_new).any(): print(f"Before nan at time {t[i]} with Max TrZ : {np.max((k1Z)[:,0,0] + (k1Z)[:,1,1] )} and min TrZ : {np.min((k1Z)[:,0,0] + (k1Z)[:,1,1] )}")
        # cond[:] = np.einsum('...ii->...',Z_new) > 100
        # Z_new[cond] /= 5.0
        
        # if count > 100 : 
        #     tend = time()
        #     print(count, uerr/count, Aerr/count, DuDterr/count, grdDuDterr/count, tend-tstart)
        #     raise SystemExit
...
if restart == False:
    ...
    loadPath = curr_path/f"data_{iname}/Re_{np.round(Re,2)},dt_{dt},N_{Nx}/time_{t_restrart:.2f}/"
    if loadPath.exists() == False: 
        raise SystemExit(f"Path {loadPath} does not exist")
        dt_load = 0.01
...
try:
    loadPath = curr_path/f"data_{iname}/Re_{np.round(Re,2)},dt_{dt},N_{Nx}/alpha_{alph}_prtcl/St_{st/tf:.2f}/time_{t_restrart:.2f}"
...
# DuDt_mat[:] = umat[...,None,0]*Amat[...,0] + umat[...,None,1]*Amat[...,1]
# # t2 = time()
# # print(f"Time taken for 10 interpolations : {t2-t1}")
